...
#		printf STDERR "where is $source.c or $source.S?\n";
#
...
sub list_multiply_defined
{
	...
	foreach $name (keys(%def)) {
		if ($#{$def{$name}} > 0) {
			# Special cases
			if ($def{$name}[0] eq "lib/memcpy.o"	# FIXME: should cjeck the 'weak' type
			||  $def{$name}[1] eq "lib/memcpy.o") {
				next;
			}
...
{
	my ($object, $type, $name, $i, $j, $export);
	printf "\n";
	foreach $object (keys(%nmdata)) {
		...
		for ($i = 0; $i <= $#{$nmdata}; ++$i) {
			...
			if ($type eq "U" || $type eq "w") {
				...
				elsif (    $name ne "__bss_start"
					&& $name ne "__bss_end"
					&& $name ne "__vectors_start"
					&& $name ne "__vectors_end"
					&& $name ne "__INITCALL_LIST__"
					&& $name ne "__CTOR_LIST__"
				) {
